...
parser.add_argument('--labelsmooth', action='store_true',default=True, help="label smooth")
parser.add_argument('--optim', type=str, default=optim, help="sgd，adam，rmsprop，optimization algorithm (see optimizers.py)")
parser.add_argument('--max-epoch', default=100, type=int,
                    help="maximum epochs to run")#训练轮数
...
# Architecture
parser.add_argument('-a', '--arch', type=str, default='resnet50', choices=models.get_names())#使用什么骨干网
# Miscs
parser.add_argument('--print-freq', type=int, default=100, help="print frequency")
parser.add_argument('--seed', type=int, default=620, help="manual seed")
#parser.add_argument('--resume', type=str, default='log/checkpoint_ep300.pth.tar', metavar='PATH')
parser.add_argument('--resume', type=str, default='', metavar='PATH')#读取模型的地方继续训练
parser.add_argument('--evaluate',default=False,action='store_true', help="evaluation only")#改成true就是评估，空值就是训练
#parser.add_argument('--evaluate',default='False',action='store_true', help="evaluation only")#改成true就是评估，空值就是训练
parser.add_argument('--eval-step', type=int, default=10,
                    help="run evaluation for every N epochs (set to -1 to test after training)")#每十轮保存一次
parser.add_argument('--start-eval', type=int, default=0, help="start to evaluate after specific epoch")
parser.add_argument('--save-dir', type=str, default='log/'+dataset+"_"+optim)#保存模型的地方
parser.add_argument('--use_cpu', action='store_true', help="use cpu")
parser.add_argument('--gpu-devices', default='0', type=str, help='gpu device ids for CUDA_VISIBLE_DEVICES')
parser.add_argument('--reranking',default=False,action= 'store_true', help= 'result re_ranking')

parser.add_argument('--test_distance',type = str, default='global', help= '可选的有global,local,global_local,test distance type')
parser.add_argument('--unaligned',action= 'store_true',default=False, help= 'test local feature with unalignment')
#控制台使用这个
...
def test(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20]):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (imgs, pids, camids) in enumerate(queryloader):
            ...
            features, local_features = model(imgs)#这里的features和local_features是什么意思？
            batch_time.update(time.time() - end)
